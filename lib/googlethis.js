...
async function search(query, options = {}) {
  ...
  options.scrapeAPI = options.scrapeAPI || '';

  const url = options.scrapeAPI+ encodeURI(options.ris &&
...
async function image(query, options = {}) {
  ...
  const url = options.scrapeAPI + encodeURI(Constants.URLS.GIS + 'search?tbm=isch' + (options.safe && '&safe=active' || '') + '&q=' + query) +
...
async function getTopNews(options = {}) {
  ...
  const url = options.scrapeAPI + Constants.URLS.GOOGLE_NEWS + 'topstories?tab=in&hl=en-US&gl=US&ceid=US:en';
